 ...
 * @param {number} x1 Coordenada x del primer punto de la línea.
 * @param {number} y1 Coordenada y del primer punto de la línea.
 * @param {number} x2 Coordenada x del segundo punto de la línea.
 * @param {number} y2 Coordenada y del segundo punto de la línea.
 * @returns {boolean} `true` si la línea intersecta el círculo, `false` en caso contrario.
 */
function intersectaLineaCirculo(cx, cy, r, x1, y1, x2, y2) {
  // Calcula la distancia entre el centro del círculo y el primer punto de la línea.
  const dx = x2 - x1;
  const dy = y2 - y1;
  const a = dx * dx + dy * dy;
